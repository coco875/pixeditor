...
class BackgroundDialog(QtWidgets.QDialog):
    def __init__(self, color=QtGui.QColor(150, 150, 150), arg=16):
        ...
        self.modImgList = QtWidgets.QStandardItemModel(0, 1)
        for f in os.listdir(os.path.join("resources", "pattern")):
            if f.endswith(".png"):
                i = QtWidgets.QStandardItem(f)
                i.path = os.path.join("resources", "pattern", f)
                self.modImgList.appendRow(i)

        ### listview to display images ###
        self.imgList = QtWidgets.QListView()
        self.imgList.setModel(self.modImgList)
        self.imgList.setSelectionMode(QtWidgets.QAbstractItemView.SingleSelection)
